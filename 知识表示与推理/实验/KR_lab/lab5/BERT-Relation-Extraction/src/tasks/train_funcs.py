...
def task_run_name(args):
    entity_representation = getattr(args, 'entity_representation', 'entity_start')
    return "%d_%s" % (args.model_no, entity_representation)
...
def load_state(net, optimizer, scheduler, args, load_best=False):
    ...
    checkpoint_path = os.path.join(base_path,"task_test_checkpoint_%s.pth.tar" % run_name)
    best_path = os.path.join(base_path,"task_test_model_best_%s.pth.tar" % run_name)
...
def load_results(args):
    ...
    run_name = task_run_name(args)
    losses_path = "./data/task_test_losses_per_epoch_%s.pkl" % run_name
    accuracy_path = "./data/task_train_accuracy_per_epoch_%s.pkl" % run_name
    f1_path = "./data/task_test_f1_per_epoch_%s.pkl" % run_name
    if os.path.isfile(losses_path) and os.path.isfile(accuracy_path) and os.path.isfile(f1_path):
        losses_per_epoch = load_pickle("task_test_losses_per_epoch_%s.pkl" % run_name)
        accuracy_per_epoch = load_pickle("task_train_accuracy_per_epoch_%s.pkl" % run_name)
        f1_per_epoch = load_pickle("task_test_f1_per_epoch_%s.pkl" % run_name)
