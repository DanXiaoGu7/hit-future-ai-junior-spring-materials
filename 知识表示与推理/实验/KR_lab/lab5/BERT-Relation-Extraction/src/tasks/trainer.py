...
from .train_funcs import load_state, load_results, evaluate_, evaluate_results, task_run_name
...
def train_and_fit(args):
    ...
    if args.model_no == 0:
        ...
        net = Model.from_pretrained(model, force_download=False, \
                                ...
                                task='classification' if args.task != 'fewrel' else 'fewrel',\
                                n_classes_=args.num_classes,
                                entity_representation=getattr(args, 'entity_representation', 'entity_start'),
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        if accuracy_per_epoch[-1] > best_pred:
            ...
            torch.save({
                ...
                }, os.path.join("./data/" , "task_test_model_best_%s.pth.tar" % run_name))
        
        if (epoch % 1) == 0:
            save_as_pickle("task_test_losses_per_epoch_%s.pkl" % run_name, losses_per_epoch)
            save_as_pickle("task_train_accuracy_per_epoch_%s.pkl" % run_name, accuracy_per_epoch)
            save_as_pickle("task_test_f1_per_epoch_%s.pkl" % run_name, test_f1_per_epoch)
            torch.save({
                    'epoch': epoch + 1,\
                    'state_dict': net.state_dict(),\
                    'best_acc': accuracy_per_epoch[-1],\
                    ...
                    'scheduler' : scheduler.state_dict(),\
                    'amp': amp.state_dict() if amp is not None else amp
                }, os.path.join("./data/" , "task_test_checkpoint_%s.pth.tar" % run_name))
    ...
    plt.savefig(os.path.join("./data/" ,"task_loss_vs_epoch_%s.png" % run_name))
    ...
    plt.savefig(os.path.join("./data/" ,"task_train_accuracy_vs_epoch_%s.png" % run_name))
    ...
    plt.savefig(os.path.join("./data/" ,"task_test_f1_vs_epoch_%s.png" % run_name))
