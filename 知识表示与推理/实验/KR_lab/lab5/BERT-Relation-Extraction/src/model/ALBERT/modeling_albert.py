...
class AlbertModel(AlbertPreTrainedModel):
    ...
    def __init__(self, config, model_size, task=None, n_classes_=None,
                 entity_representation='entity_start'):
        super().__init__(config)
        ...
        if self.task is None:
            ...
            if self.entity_representation == 'cls':
                representation_size = config.hidden_size
            else:
                representation_size = config.hidden_size * 2
            self.classification_layer = nn.Linear(representation_size, n_classes_)
    ...
    Example::
        ...
        if self.task == 'classification' and self.entity_representation == 'cls':
            v1v2 = sequence_output[:, 0, :]
        else:
            if torch.is_tensor(e1_e2_start):
                e1_e2_start = e1_e2_start.tolist()
            buffer = []
            for i in range(sequence_output.shape[0]):
                e1_start, e2_start = e1_e2_start[i]
                v1v2 = torch.cat((sequence_output[i, e1_start, :],
                                   sequence_output[i, e2_start, :]))
                buffer.append(v1v2)
