...
class DistilBertForSequenceClassification(DistilBertPreTrainedModel):
    def __init__(self, config: PretrainedConfig):
        ...
        # Allow switching sentence representation without editing the model each time.
        # Valid values: "cls" (default), "mean", "max".
        self.config.pooling_type = getattr(config, "pooling_type", "cls")
    ...
    def forward(
        ...
        pooling_type = getattr(self.config, "pooling_type", "cls")
        # 1) CLS pooling (original): pooled_output = hidden_state[:, 0]
        # 2) Mean pooling (mask-aware): uncomment by setting pooling_type="mean"
        # 3) Max pooling (mask-aware): uncomment by setting pooling_type="max"
        if pooling_type == "mean":
            if attention_mask is None:
                pooled_output = hidden_state.mean(dim=1)
            else:
                # Expand mask for broadcasting then average only valid tokens
                mask = attention_mask.unsqueeze(-1).type_as(hidden_state)
                pooled_output = (hidden_state * mask).sum(dim=1) / mask.sum(dim=1).clamp(min=1e-6)
        elif pooling_type == "max":
            if attention_mask is None:
                pooled_output = hidden_state.max(dim=1).values
            else:
                mask = attention_mask.unsqueeze(-1)
                pooled_output = hidden_state.masked_fill(mask == 0, float("-inf")).max(dim=1).values
        else:
            pooled_output = hidden_state[:, 0]  # default CLS token
