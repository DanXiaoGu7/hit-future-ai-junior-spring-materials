...
//×ÓœÚµãžöÊý²»È·¶š ÐèÒªÓÃµœstdarg.hÏà¹ØÄÚÈÝ
static Treenode newnode(int line,char *TOKEN,int amount,...)//amountÎª×ÓœÚµãžöÊý Ò²ÊÇÊ¡ÂÔºÅÖÐ²ÎÊýµÄÊýÁ¿
{
	...
	if (amount==0)
	{
		rt->isleaf=1;
		rt->firstchild=NULL;
		if((!strcmp(TOKEN,"ID"))||(!strcmp(TOKEN,"TYPE"))||(!strcmp(TOKEN,"RELOP")))
		{
			char *str;
			...
			rt->intval=atoi(yytext);
		else if(!strcmp(TOKEN,"FLOAT"))
			rt->floatval=atof(yytext);
	}
	else
	{
		va_list list;//×ÓœÚµãÁÐ±í
		va_start(list,amount);//µÚÒ»žö×ÓœÚµã
