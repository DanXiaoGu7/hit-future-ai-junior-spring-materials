...
InterCode create_intercode(enum InterCodeKind kind, int intercode_type, ...) {
    ...
    va_list valist;
    va_start(valist, (intercode_type % 10));  // 开始访问可变参数列表

    // 根据intercode_type的值，决定如何从参数列表中读取数据并填充到InterCode结构体中
    if (intercode_type == INTERCODE_1) {
        // 对于INTERCODE_1类型，期望有一个操作数
        ic->operand_1.op = va_arg(valist, Operand);  // 从参数列表中获取一个Operand
    }
    else if (intercode_type == INTERCODE_2) {
        // 对于INTERCODE_2类型，期望有两个操作数
        ic->operand_2.op1 = va_arg(valist, Operand); // 从参数列表中获取第一个Operand
        ic->operand_2.op2 = va_arg(valist, Operand); // 从参数列表中获取第二个Operand
    }
    else if (intercode_type == INTERCODE_3) {
        // 对于INTERCODE_3类型，期望有三个操作数（一个结果和两个输入）
        ic->operand_3.result = va_arg(valist, Operand); // 从参数列表中获取结果操作数
        ic->operand_3.op1 = va_arg(valist, Operand);    // 从参数列表中获取第一个输入操作数
        ic->operand_3.op2 = va_arg(valist, Operand);    // 从参数列表中获取第二个输入操作数
    }
    else if (intercode_type == INTERCODE_IF) {
        // 对于INTERCODE_IF类型，期望有四个操作数（用于条件跳转）
        ic->operand_if.x = va_arg(valist, Operand);      // 条件左边的操作数
        ic->operand_if.relop = va_arg(valist, Operand);  // 关系操作符
        ic->operand_if.y = va_arg(valist, Operand);      // 条件右边的操作数
        ic->operand_if.z = va_arg(valist, Operand);      // 跳转的标签
    }
    else if (intercode_type == INTERCODE_DEC) {
        // 对于INTERCODE_DEC类型，期望有一个操作数和一个整数（用于数组或内存分配）
        ic->operand_dec.op = va_arg(valist, Operand);    // 操作数，通常是变量
        ic->operand_dec.size = va_arg(valist, int);      // 数组或分配的大小
    }
...
void translate_ExtDefList(Treenode now)
{
    ...
    if (now != NULL && now->Token != NULL){
        ...
        translate_ExtDefList(now->firstchild->nxxtbro);
    }
}
...
void translate_ExtDef(Treenode now)
{
    ...
    */
    ...
    Treenode specifier = now->firstchild;
    if (strcmp(specifier->nxxtbro->Token, "SEMI") == 0 || strcmp(specifier->nxxtbro->Token, "ExtDecList") == 0)
    {
        return;
    }
    if (strcmp(specifier->nxxtbro->Token, "FunDec") == 0)
    {
        translate_FunDec(specifier->nxxtbro);
        translate_CompSt(specifier->nxxtbro->nxxtbro);
    }
}
...
void translate_FunDec(Treenode now)
{
    ...
    if (now->firstchild->nxxtbro->nxxtbro->nxxtbro == NULL)
        return; //ID LP RP
...
void translate_DefList(Treenode now)
{
    ...
    Treenode def = now->firstchild;
    while (def!=NULL && def->Token!=NULL)
    {
        translate_Def(def);
        def = def->nxxtbro->firstchild;
    }
}
...
void translate_StmtList(Treenode now)
{
    ...
    Treenode child = now->firstchild;
    if (child != NULL && child->Token!=NULL)
    {
        translate_Stmt(child);
        translate_StmtList(child->nxxtbro);
    }
}
...
void translate_Def(Treenode now)
{
    ...
    translate_DecList(now->firstchild->nxxtbro);
}
// 语句
void translate_Stmt(Treenode now)
{
    ...
    else if (strcmp(child->Token, "RETURN") == 0)
    {
        ...
        Operand t1 = new_temp();
        translate_Exp(now->firstchild->nxxtbro, t1);
    ...
    else if (strcmp(child->Token, "IF") == 0)
    {
        ...
        Treenode else_node = child->nxxtbro->nxxtbro->nxxtbro->nxxtbro->nxxtbro;
        if (else_node == NULL)
        {
            // Stmt →IF LP Exp RP Stmt
            Operand l1 = new_label();
            Operand l2 = new_label();
            Treenode exp = child->nxxtbro->nxxtbro;
            Treenode stmt = exp->nxxtbro->nxxtbro;
        ...
        else
        {
            ...
            Treenode exp = child->nxxtbro->nxxtbro;
            Treenode stmt = exp->nxxtbro->nxxtbro;
            Treenode else_stmt = stmt->nxxtbro->nxxtbro;
...
void translate_DecList(Treenode now)
{
    ...
    Treenode child = now->firstchild;
    translate_Dec(child);
    Treenode comma = child->nxxtbro;
    if (comma)
        translate_DecList(comma->nxxtbro);
}
// 表达式
void translate_Exp(Treenode now, Operand place)
{
    Treenode child = now->firstchild;
    Treenode child2 = child->nxxtbro;
    Treenode child3;
    if (child2)
        child3 = child2->nxxtbro;
    if (child2 == NULL)
    {
        if (place == NULL)
                return;
    ...
    else if (strcmp(child->Token, "MINUS") == 0)
    {
        ...
        if (place == NULL)
            return;
        Operand t1 = new_temp();
        translate_Exp(child->nxxtbro, t1);
        place->kind = VARIABLE;
    ...
    else if (strcmp(child2->Token, "ASSIGNOP") == 0)
    {
        ...
        Treenode child_child = child->firstchild;
    ...
    else if (strcmp(child2->Token, "PLUS") == 0 || strcmp(child2->Token, "MINUS") == 0 || strcmp(child2->Token, "STAR") == 0 || strcmp(child2->Token, "DIV") == 0)
    {
        ...
        translate_Exp(now->firstchild->nxxtbro->nxxtbro, t2);
        Operand t3 = t1, t4 = t2;
...
// 条件
void translate_Cond(Treenode now, Operand ltrue, Operand lfalse)
{
    Treenode child = now->firstchild;

    if (child->nxxtbro != NULL && strcmp(child->nxxtbro->Token, "RELOP") == 0)
    {
        Operand t1 = new_temp();
        Operand t2 = new_temp();
        
        translate_Exp(child, t1);
        translate_Exp(child->nxxtbro->nxxtbro, t2);
        
        Operand op = create_operand(RELOP,OPERAND_NULL,child->nxxtbro->Id_Type);
    ...
    {
        translate_Cond(now->firstchild->nxxtbro, lfalse, ltrue);
    }
    else if (child->nxxtbro != NULL && strcmp(child->nxxtbro->Token, "AND") == 0)
    {
        Operand l1 = new_label();
        translate_Cond(child, l1, lfalse);
        insert_intercode(create_intercode(ILABEL, INTERCODE_1, l1));
        translate_Cond(child->nxxtbro->nxxtbro, ltrue, lfalse);
    }
    else if (child->nxxtbro != NULL && strcmp(child->nxxtbro->Token, "OR") == 0)
    {
        Operand l1 = new_label();
...
void translate_Dec(Treenode now)
{
    ...
    Treenode vardec = now->firstchild;
    Treenode child2 = vardec->nxxtbro;
    if (child2 == NULL)
    {
        Operand t1 = new_temp();
    ...
    else
    {
        ...
        Operand t2 = new_temp();
        translate_VarDec(vardec, t1);
        translate_Exp(child2->nxxtbro, t2);
...
void translate_Args(Treenode now, InterCode here)
{
    ...
    Treenode comma = exp->nxxtbro;
    if (comma)
        translate_Args(comma->nxxtbro, code1_in);
