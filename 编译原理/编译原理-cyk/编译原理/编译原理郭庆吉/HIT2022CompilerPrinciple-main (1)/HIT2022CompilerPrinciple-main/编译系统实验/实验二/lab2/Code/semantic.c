...
//这个函数的作用是根据语法规则解析结构体声明，并生成对应的类型结构体。如果结构体是一个新定义的，则将其添加到符号表中；如果结构体引用了一个已定义的结构体，则从符号表中查找其定义。
