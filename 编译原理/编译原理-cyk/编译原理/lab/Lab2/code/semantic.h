...
typedef struct fieldList {// 域列表
    char* name;       // 域的名字
    pType type;       // 域的类型
    pFieldList tail;  // 下一个域
} FieldList;

typedef struct type { // 数据类型
    Kind kind;// 四种类型之一
    // 确定数据存储的大小
    union {
        // 基本类型
        ...
        struct {
            char* structName;
            pFieldList field; // 域名，类型，下一个
        } structure;
        // 函数：参数个数、参数类型、返回值类型
        struct {
            int argc;          // argument counter
            pFieldList argv;   // 链式参数列表
...
typedef struct tableItem {
    int symbolDepth;    // 符号所在的作用域深度
    pFieldList field;   // 符号的内容
    pItem nextSymbol;  // 同一作用域下下一个符号
    pItem nextHash;    // 同一哈希冲突表的下一个项
} TableItem;
...
typedef struct stack {
    pItem* stackArray; // 每一个作用域的符号链表
    int curStackDepth; // 作用域深度
} Stack;

typedef struct table {
    pHash hash; // 哈希表
    pStack stack; // 作用域栈
    int unNamedStructNum; //匿名结构体号
