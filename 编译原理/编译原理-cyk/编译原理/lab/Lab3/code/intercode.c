...
// 向指令链表interCodes尾部插入多条指令组成的双向链表code
...
void printInterCodes(char* name) {
    ...
    while (flag == 1 || curr != interCodes) {
        flag = 0;
        switch(curr->kind) { //根据kind的值来打印对应的指令
            case LABEL_IR:
                fputs("LABEL ", fp);
...
// 创建变量操作数
Operand getVar(char* name) { //程序中的变量
...
InterCode translateExp(Node* root, Operand place) {
    // 赋值表达式
    if (root->childNum == 3 && strcmp(root->children[1]->name, "ASSIGNOP") == 0) { //Exp -> Exp ASSIGNOP Exp
        // 单个变量作为左值
        ...
        // 数组元素作为左值
            ...
            strcmp(root->children[0]->children[1]->name, "LB") == 0) { // Exp -> Exp    ASSIGNOP   Exp
                                                                       //        |-> Exp LB Exp RB   
                                                                       //            a    [  i  ]
            ...
            InterCode code1 = translateExp(root->children[0]->children[0], tmp1); // t1 := &a 
            InterCode code2 = translateExp(root->children[0]->children[2], tmp2); // t2 := i
            int size = getSize(tmp1->type->array.elem);
            InterCode code3 = optimizeMULIR(tmp3, tmp2, getValue(size));          // t3 := t2 * size 
            insertInterCode(code2, code1);                                           
            insertInterCode(code3, code1);
            // tmp4存储所取元素的首地址
            Operand tmp4 = newTemp();
            InterCode code4 = optimizePLUSIR(tmp4, tmp1, tmp3);                   // t4 := t1 + t3
            insertInterCode(code4, code1);                                        
            // tmp5存储的是右侧表达式的运算结果
            Operand tmp5 = newTemp();
            InterCode code5 = translateExp(root->children[2], tmp5);              // t5 is the result of Exp
            InterCode code6 = (InterCode)malloc(sizeof(InterCode_));              // *t4 := t5
        ...
        // 结构体特定域作为左值
            ...
            strcmp(root->children[0]->children[1]->name, "DOT") == 0) { // Exp -> Exp    ASSIGNOP   Exp
                                                                        //        |-> Exp DOT ID   
                                                                        //             a   .  f
            // 获取域名
            char name[32];
            strcpy(name, root->children[0]->children[2]->strVal); // name = f
            Operand tmp1 = newTemp();
            ...
            while (strcmp(head->name, name) != 0) { //get the offset of f
                int tmp = getSize(head->type);
                // 按四字节对齐
                if (tmp % 4 != 0)
                    tmp = ((tmp / 4) + 1) * 4;
            ...
            // tmp2中存储域的首地址
            Operand tmp2 = newTemp();
            InterCode code2 = optimizePLUSIR(tmp2, tmp1, getValue(offset));        // t2 := t1 + offset
            insertInterCode(code2, code1);
            // tmp3存储的是右侧表达式的运算结果
            Operand tmp3 = newTemp();
            InterCode code3 = translateExp(root->children[2], tmp3);               // t3 is the result of Exp
            InterCode code4 = (InterCode)malloc(sizeof(InterCode_));               // *t2 := t3
            ...
            // 将运算结果存回place
            if (place != NULL)
                operandCpy(place, getVal(tmp2));                                    // final result in place
            return code1;
        }
    }
    // 加减乘除表达式
    else if (root->childNum == 3 && ( // Exp -> Exp1 OP Exp2
        ...
        Operand tmp1 = newTemp();
        Operand tmp2 = newTemp();
        InterCode code1 = translateExp(root->children[0], tmp1); // t1 is the result of Exp1
        InterCode code2 = translateExp(root->children[2], tmp2); // t2 is the result of Exp2
        insertInterCode(code2, code1);
        InterCode code3 = getNullInterCode();
        switch (root->children[1]->name[0]) {
            case 'P': code3 = optimizePLUSIR(place, tmp1, tmp2); break; // place := t1 OP t2
    ...
    // 取负表达式
    else if (strcmp(root->children[0]->name, "MINUS") == 0) { // Exp ->  MINUS Exp
        Operand tmp1 = newTemp();
        InterCode code1 = translateExp(root->children[1], tmp1);   // t1 is the result of Exp
        InterCode code2 = optimizeSUBIR(place, getValue(0), tmp1); // place := #0 - t1
        insertInterCode(code2, code1);
        return code1;
    }
    // 括号表达式
    else if (strcmp(root->children[0]->name, "LP") == 0) { // Exp -> LP Exp RP
        Operand tmp1 = newTemp();
        InterCode code1 = translateExp(root->children[1], tmp1); // t1 is the result of Exp
        // 优化：直接把place修改为tmp1
        if (place != NULL)
            operandCpy(place, tmp1); // final result in place
        return code1;
    }
    // 条件表达式
    else if (root->childNum >= 2 && (
             strcmp(root->children[0]->name, "NOT") == 0 ||   // Exp -> NOT Exp
             strcmp(root->children[1]->name, "RELOP") == 0 || // Exp -> Exp1 RELOP Exp2
             strcmp(root->children[1]->name, "AND") == 0 ||   // Exp -> Exp1 AND Exp2
             strcmp(root->children[1]->name, "OR") == 0)) {   // Exp -> Exp1 OR Exp2
        Operand label1 = newLabel();
        Operand label2 = newLabel();
        InterCode code1 = (InterCode)malloc(sizeof(InterCode_)); // place := #0
        code1->kind = ASSIGN_IR;
        code1->ops[0] = place;
        code1->ops[1] = getValue(0);
        InterCode code2 = translateCond(root, label1, label2);   // IF root GOTO label1
        optimizeLABELBeforeGOTO(code2, label1);                  // GOTO label2
        InterCode code3 = (InterCode)malloc(sizeof(InterCode_)); // LABLE label1:
        code3->kind = LABEL_IR;                                     
        code3->ops[0] = label1;
        InterCode code4 = (InterCode)malloc(sizeof(InterCode_)); // place := #1
        code4->kind = ASSIGN_IR;
        code4->ops[0] = place;
        code4->ops[1] = getValue(1);
        InterCode code5 = (InterCode)malloc(sizeof(InterCode_)); // LABEL label2:
        code5->kind = LABEL_IR;                                  
        code5->ops[0] = label2;
        insertInterCode(code2, code1);
        insertInterCode(code3, code1);
    ...
    else if (strcmp(root->children[0]->name, "ID") == 0) { // Exp -> ID | ID LP RP | ID LP Args RP
        // 单变量表达式
        if (root->childNum == 1) { // Exp -> ID
            Entry sym = findSymbolAll(root->children[0]->strVal); //查符号表来判断类型
            Operand var = getVar(sym->name);
            // 数组类型和结构体类型并且不是函数参数是局部变量——需要取地址指令
            if (sym->type->kind != ENUM_BASIC && sym->isArg == 0) {
                operandCpy(place, getAddr(var)); //数组和结构体变量需要取地址
                place->type = sym->type;
                return getNullInterCode();
            }
            else { // 基本类型或函数参数，直接返回值
                operandCpy(place, var);
                place->type = sym->type;
                return getNullInterCode();
            }
        ...
        // 函数调用表达式
            ...
            if (root->childNum == 3) {
                ...
                if (strcmp(func->name, "read") == 0) {
                    InterCode code1 = (InterCode)malloc(sizeof(InterCode_)); // READ place
                    code1->kind = READ_IR;
                    code1->ops[0] = place;
                    return code1;
                }
                InterCode code1 = (InterCode)malloc(sizeof(InterCode_)); // place := CALL func
                code1->kind = CALL_IR;
                code1->ops[0] = place;
                code1->ops[1] = func;
                return code1;
            ...
            // 带参函数
                ...
                if (strcmp(func->name, "write") == 0) {
                    InterCode code2 = (InterCode)malloc(sizeof(InterCode_)); // WRITE arg
                    code2->kind = WRITE_IR;
                ...
                Operand curr = argList->next; //遍历参数链表，由于是头插法，所以参数链表是倒序的
                while (curr != NULL) {
                    InterCode code2 = (InterCode)malloc(sizeof(InterCode_)); // ARG argn ... arg1
                    code2->kind = ARG_IR;
                    code2->ops[0] = curr;
                    insertInterCode(code2, code1);
                    curr = curr->next;
                }
                InterCode code3 = (InterCode)malloc(sizeof(InterCode_)); // place := CALL func
                code3->kind = CALL_IR;  
                code3->ops[0] = place;
                code3->ops[1] = func;
                insertInterCode(code3, code1);
    ...
    // 数组元素表达式
    else if (root->childNum == 4 && strcmp(root->children[1]->name, "LB") == 0) { // Exp -> Exp LB Exp RB
        // tmp1应当是地址，tmp2应当是一个整型                                                   a  [  i   ]
        Operand tmp1 = newTemp();
        Operand tmp2 = newTemp();
        Operand tmp3 = newTemp();
        InterCode code1 = translateExp(root->children[0], tmp1); // t1 := &a
        InterCode code2 = translateExp(root->children[2], tmp2); // t2 := i
        insertInterCode(code2, code1);
        // place为NULL没必要继续算
        if (place != NULL) {
            int size = getSize(tmp1->type->array.elem);          
            ...
            if (tmp1->type->array.elem->kind == ENUM_ARRAY || 
                ...
                // 优化加法
                InterCode code4 = optimizePLUSIR(place, tmp1, tmp3); // place := t1 + t3
                insertInterCode(code4, code1);
            ...
            // 取出的数组元素是一个基本类型，则返回值
                ...
                InterCode code4 = optimizePLUSIR(tmp4, tmp1, tmp3); // t4 := t1 + t3
                insertInterCode(code4, code1);
                operandCpy(place, getVal(tmp4));                    // place := *t4
            }
            return code1;
        }
        return code1;
    ...
    // 取结构体域
    else if (root->childNum == 3 && strcmp(root->children[1]->name, "DOT") == 0) { // Exp -> Exp DOT ID
                                                                                   //         a   .  f  
        // 获取域名
        char name[32];
        strcpy(name, root->children[2]->strVal); // name = f
        Operand tmp1 = newTemp();
        // tmp1返回的是一个结构体的首地址，并且带有type属性
        InterCode code1 = translateExp(root->children[0], tmp1); // t1 := &a
        // 获取域的偏移量和类型
        int offset = 0;
        FieldList head = tmp1->type->structure->head;            // head是结构体的域链表
        while (strcmp(head->name, name) != 0) {                  // get the offset of f
            int tmp = getSize(head->type);
            // 按四字节对齐
            if (tmp % 4 != 0)
        ...
        if (place != NULL) {
            ...
            if (head->type->kind == ENUM_BASIC) { // 如果取出的域是一个基本类型，则返回值
                Operand tmp2 = newTemp();
                code2 = optimizePLUSIR(tmp2, tmp1, getValue(offset)); // t2 := t1 + offset
                insertInterCode(code2, code1);
                operandCpy(place, getVal(tmp2));                      // place := *t2
            }
            else {
...
InterCode translateArgs(Node* root, Operand argList) {
    if (root->childNum == 1) { //  Args -> Exp
        Operand tmp1 = newTemp();
        InterCode code1 = translateExp(root->children[0], tmp1); // t1 is the result of Exp
        tmp1->next = argList->next;//插在参数链表的头部
        argList->next = tmp1;
        return code1;
    }
    else if (root->childNum == 3) { // Args -> Exp COMMA Args
        Operand tmp1 = newTemp();
...
InterCode translateStmt(Node* root) { 
    if (strcmp(root->children[0]->name, "Exp") == 0) {// Stmt -> Exp SEMI
        return translateExp(root->children[0], NULL); // 直接翻译Exp
    }
    else if (strcmp(root->children[0]->name, "CompSt") == 0) { // Stmt -> CompSt
        pushLayer(); // 进入新作用域
        InterCode code1 = translateCompSt(root->children[0], NULL); 
        popLayer();
        return code1;
    }
    else if (strcmp(root->children[0]->name, "RETURN") == 0) { // Stmt -> Return Exp SEMI
        Operand tmp1 = newTemp();
        InterCode code1 = translateExp(root->children[1], tmp1); // t1 is the result of Exp
        InterCode code2 = (InterCode)malloc(sizeof(InterCode_)); // RETURN t1
        code2->kind = RETURN_IR;
        code2->ops[0] = tmp1;
        insertInterCode(code2, code1);
        return code1;
    }
    else if (strcmp(root->children[0]->name, "IF") == 0 && root->childNum == 5) { // Exp -> IF LP Exp RP Stmt
        Operand label1 = newLabel();
        Operand label2 = newLabel();
        InterCode code1 = translateCond(root->children[2], label1, label2);       // IF Exp GOTO label1
                                                                                  // GOTO label2 
        InterCode code2 = (InterCode)malloc(sizeof(InterCode_));                  // LABEL label1:
        code2->kind = LABEL_IR;
        code2->ops[0] = label1;
        InterCode code3 = translateStmt(root->children[4]);                       // Stmt
        InterCode code4 = (InterCode)malloc(sizeof(InterCode_));                  // LABEL label2:
        code4->kind = LABEL_IR;
        code4->ops[0] = label2;
        insertInterCode(code2, code1);
        if (code3 != NULL) // CompSt -> e
            insertInterCode(code3, code1);
        insertInterCode(code4, code1);
        return code1;
    }  
    else if (strcmp(root->children[0]->name, "IF") == 0 && root->childNum == 7) { // Exp -> IF LP Exp RP Stmt1 ELSE Stmt2
        Operand label1 = newLabel();
        Operand label2 = newLabel();
        ...
        if (last->kind == GOTO_IR && last->ops[0]->no == label2->no) { // code1.last: GOTO label2 and only one "GOTO label2"
            // 那么优化成：
            // IF Exp GOTO label1
            // Stmt2
            // GOTO label3
            ...
            if (count == 1) { // label2只被引用一次
                last->kind = NULL_IR;   // 将label2的GOTO语句删除
                InterCode code2 = translateStmt(root->children[6]);                                         // Stmt2
                // 优化：如果code2的最后一句是LABEL语句，那么将code2中的所有GOTO语句中的该LABEL替换为LABEL3
                optimizeLABELBeforeGOTO(code2, label3);
                // Stmt: ...
                //       label2:   ->    delete label2 and change all GOTO tagets into label3
                // GOTO  label3                                                    
                InterCode code3 = (InterCode)malloc(sizeof(InterCode_));                                    // GOTO label3
                code3->kind = GOTO_IR;
                code3->ops[0] = label3;
                InterCode code4 = (InterCode)malloc(sizeof(InterCode_));                                    // LABEL label1:
                code4->kind = LABEL_IR;
                code4->ops[0] = label1;
                InterCode code5 = translateStmt(root->children[4]);                                         // Stmt1
                optimizeLABELBeforeGOTO(code5, label3);
                InterCode code6 = (InterCode)malloc(sizeof(InterCode_));                                    // LABEL label3:
                code6->kind = LABEL_IR;
                code6->ops[0] = label3;
                insertInterCode(code2, code1);
                insertInterCode(code3, code1);
    ...
    else if (strcmp(root->children[0]->name, "WHILE") == 0) {// Exp -> WHILE LP Exp RP Stmt
        Operand label1 = newLabel();
        Operand label2 = newLabel();
        Operand label3 = newLabel();
        InterCode code1 = (InterCode)malloc(sizeof(InterCode_));            // LABEL label1:
        code1->kind = LABEL_IR;
        code1->ops[0] = label1;
        InterCode code2 = translateCond(root->children[2], label2, label3); // IF Exp GOTO label2
                                                                            // GOTO label3
        InterCode code3 = (InterCode)malloc(sizeof(InterCode_));            // LABEL label2:
        code3->kind = LABEL_IR;
        code3->ops[0] = label2;
        InterCode code4 = translateStmt(root->children[4]);                 // Stmt
        optimizeLABELBeforeGOTO(code4, label1);
        InterCode code5 = (InterCode)malloc(sizeof(InterCode_));            // GOTO label1
        code5->kind = GOTO_IR;
        code5->ops[0] = label1;
        InterCode code6 = (InterCode)malloc(sizeof(InterCode_));            // LABEL label3:
        code6->kind = LABEL_IR;
...
InterCode translateCond(Node* root, Operand labelTrue, Operand labelFalse) {
    if (root->childNum >= 2 && strcmp(root->children[1]->name, "RELOP") == 0) { //Exp -> Exp RELOP Exp
        Operand tmp1 = newTemp();
        Operand tmp2 = newTemp();
        InterCode code1 = translateExp(root->children[0], tmp1); // t1 
        InterCode code2 = translateExp(root->children[2], tmp2); // t2 
        InterCode code3 = (InterCode)malloc(sizeof(InterCode_)); // IF t1 relop t2 GOTO labelTrue
        ...
        InterCode code4 = (InterCode)malloc(sizeof(InterCode_));
        code4->kind = GOTO_IR;                                   // GOTO labelFalse
    ...
    else if (strcmp(root->children[0]->name, "NOT") == 0) { // Exp -> NOT Exp
        return translateCond(root->children[1], labelFalse, labelTrue); // 互换跳转标签
    }
    else if (root->childNum >= 2 && ( 
             strcmp(root->children[1]->name, "AND") == 0 || // Exp -> Exp1 AND Exp2
             strcmp(root->children[1]->name, "OR") == 0)) { // Exp -> Exp1 OR Exp2
        ...
        InterCode code1;
        switch (root->children[1]->name[0]) { 
            case 'A': code1 = translateCond(root->children[0], label1, labelFalse); break; // IF Exp1 GOTO label1
                                                                                           // GOTO labelFalse         
            case 'O': code1 = translateCond(root->children[0], labelTrue, label1); break;  // IF Exp1 GOTO labelTrue
                                                                                           // GOTO label1
        }
        InterCode code2 = (InterCode)malloc(sizeof(InterCode_));                           // LABEL label1: 
        code2->kind = LABEL_IR;
        code2->ops[0] = label1;
        InterCode code3 = translateCond(root->children[2], labelTrue, labelFalse);         // IF Exp2 GOTO labelTrue
                                                                                           // GOTO labelFalse     
        insertInterCode(code2, code1);
        insertInterCode(code3, code1);
        return code1;
        ...
        Operand tmp1 = newTemp();
        InterCode code1 = translateExp(root, tmp1); // t1
        InterCode code2 = (InterCode)malloc(sizeof(InterCode_));
        code2->kind = IF_GOTO_IR; // IF t1 != 0 GOTO lablelTrue
        code2->ops[0] = tmp1;
        code2->ops[1] = getValue(0);
        code2->ops[2] = labelTrue;
        strcpy(code2->relop, "!=");
        InterCode code3 = (InterCode)malloc(sizeof(InterCode_));
        code3->kind = GOTO_IR;    // GOTO labelFalse
...
void translateProgram(Node* root) { // Program -> ExtDefList
    initSymbolTable();
    initInterCodes();
...
InterCode translateExtDefList(Node* root) { // ExtDefList -> ExtDef ExtDefList | e
    if (root->childNum != 0) {
        InterCode code1 = translateExtDef(root->children[0]);     // ExtDef
        InterCode code2 = translateExtDefList(root->children[1]); // ExtDefList 
...
InterCode translateExtDef(Node* root) {
    ...
    if (type->kind == ENUM_STRUCT && type->structure->name != "" && type->structure->head != NULL) {
        Entry res = (Entry)malloc(sizeof(Entry_)); // 将结构体信息注册进符号表
    ...
    if (strcmp(root->children[1]->name, "FunDec") == 0) {
        ...
        InterCode code1 = (InterCode)malloc(sizeof(InterCode_));     // FUNC name
        code1->kind = FUNC_IR;
        code1->ops[0] = getFunc(func->name);
        FieldList head = func->head;
        while (head != NULL) {
            InterCode code2 = (InterCode)malloc(sizeof(InterCode_)); // PARAM p1 ... pn
            code2->kind = PARAM_IR;
        ...
        insertSymbol(res);
        pushLayer();
        InterCode code3 = translateCompSt(root->children[2], func->name);// 翻译函数体
        popLayer();
        if (code3 != NULL)
            insertInterCode(code3, code1);
        return code1;
...
FieldList translateDec(Node* root, Type type, IdType class, InterCode code) {
    ...
    // 局部变量在声明时初始化
    if (class == ENUM_VAR && res != NULL && root->childNum == 3) { // Dec -> VarDec ASSIGNOP Exp
        Operand tmp1 = newTemp();
        InterCode code1 = translateExp(root->children[2], tmp1);    // t1 is the result of Exp
        insertInterCode(code1, code);
        InterCode code2 = (InterCode)malloc(sizeof(InterCode_));    // t2 := t1
        code2->kind = ASSIGN_IR;
